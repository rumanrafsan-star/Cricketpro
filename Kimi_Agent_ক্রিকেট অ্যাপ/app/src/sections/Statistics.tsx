...
export default function Statistics() {
  ...
  return (
    <section id="stats" className="py-20 bg-[#121212] text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center mb-12">
          <h2 className="text-5xl sm:text-6xl font-bold mb-4">
            PLAYER <span className="text-[#f2d24b]">STATISTICS</span>
          </h2>
          ...
          <CardContent className="p-8">
            <div className="grid lg:grid-cols-2 gap-8 items-center">
              <div className="space-y-6">
                <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#f2d24b]/20 rounded-full">
                  <Award className="w-5 h-5 text-[#f2d24b]" />
                  <span className="text-[#f2d24b] font-medium">Top Performer</span>
                </div>
                <div>
                  <h3 className="text-4xl font-bold mb-2">{topPlayer.name}</h3>
                  <p className="text-gray-400">{topPlayer.team} • {topPlayer.role}</p>
                </div>
                <div className="grid grid-cols-3 gap-6">
                  <div className="space-y-1">
                    <p className="text-3xl font-bold text-[#f2d24b]">{topPlayer.stats.runs.toLocaleString()}</p>
                    <p className="text-sm text-gray-400">Runs</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-3xl font-bold text-[#f2d24b]">{topPlayer.stats.average}</p>
                    <p className="text-sm text-gray-400">Average</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-3xl font-bold text-[#f2d24b]">{topPlayer.stats.strikeRate}</p>
              ...
              <div className="flex justify-center">
                <div className="relative">
                  <div className="w-48 h-48 rounded-full bg-gradient-to-br from-[#f2d24b]/30 to-[#f2d24b]/10 flex items-center justify-center">
                    <User className="w-24 h-24 text-[#f2d24b]" />
                  </div>
                  <div className="absolute -inset-4 bg-[#f2d24b]/10 rounded-full blur-2xl" />
                </div>
              </div>
            </div>
          </CardContent>
                ...
                <tbody>
                  {sortedPlayers.map((player, index) => (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 text-right font-bold text-[#f2d24b]">
